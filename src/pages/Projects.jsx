...
export default function Projects() {
  ...
  async function loadProjects() {
    ...
    const { data, error } = await supabase
      .from("projects")
      .select(
        "*, branches(name), project_images(id, image_url, display_order)"
      )
  ...
  function openEdit(item) {
    ...
    setForm({
      ...
      amount_spent:
        item.amount_spent != null ? String(item.amount_spent) : "",
  ...
  function validate() {
    ...
    if (
      form.budget &&
      (Number.isNaN(Number(form.budget)) || Number(form.budget) < 0)
    ) {
    ...
    if (
      form.amount_spent &&
      (Number.isNaN(Number(form.amount_spent)) ||
        Number(form.amount_spent) < 0)
    ) {
  ...
  async function handleSubmit(e) {
    ...
    const payload = {
      ...
      amount_spent:
        form.amount_spent === "" ? 0 : Number(form.amount_spent),
      status: form.status,
    ...
    if (error) {
      ...
      showToast(
        "error",
        "Couldn't save project. Please try again."
      );
    ...
    if (form.cover_image_url) {
      ...
      if (imageError) {
        ...
        showToast(
          "error",
          "Project saved, but the image couldn't be saved."
        );
      }
  ...
  const totalSpent = items.reduce(
    (sum, p) => sum + Number(p.amount_spent || 0),
    0
  );

  return (
    <div className="pt-4 sm:pt-0 px-4 sm:px-6 lg:px-8">
      {/* HEADER */}
      ...
      >
        <div>
          ...
          <p className="mt-2 text-sm text-ink/60 max-w-xl">
            Shown on the public Projects page. Mark a project
            "Featured" to surface it on the homepage.
          </p>
        </div>
      </motion.div>
      ...
      {!loading && !loadError && items.length > 0 && (
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 gap-3 max-w-xl">
          <div className="rounded-xl border border-ink/10 bg-white/60 p-3.5">
            ...
            <p className="font-display text-lg text-pine-dark">
              {activeCount}
            </p>
          </div>
        ...
        {!loading && loadError && (
          <div className="rounded-2xl border border-care/30 bg-care/5 p-6">
            <p className="text-sm text-care">
              Couldn't load projects. Make sure the Supabase
              migrations have been run and your .env is configured.
            </p>
          </div>
        )}
                    ...
                    <div className="flex min-w-0 items-center gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="font-medium text-pine-dark text-sm truncate">
                            {item.title}
                          </p>
        ...
        >
          <div>
            ...
            {errors.title && (
              <p
                role="alert"
                className="mt-1.5 text-xs text-care"
              >
                {errors.title}
              </p>
            )}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs text-ink/40">
                A running total — expenses recorded against
                this project don't update it automatically yet.
              </p>
            </div>
          </div>
